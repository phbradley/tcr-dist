...
with Parser(locals()) as p:
    ...
    p.flag('new_TR_databases').described_as("Create new, temporary database with most up to date TR sequences. Please see NOTES above.")
    p.flag('use_tempdb').described_as("Prevent the deletion and force the use of the most recent temporary TCR database. Please see NOTES above.")
...
#################
  ...
  TCR databases notes:
    The default TCR database was obtained from IMGT (Lefranc M-P et al. Nucleic Acids Res. 2015) prior to 
    release of the pipeline. We continue to support this database in order to provide consistency among
    analyses. However, the data underlying the TCR databases are constantly being updataed. If you would
    like to use the most up-to-date information available through IMGT, you may use the --new_TR_databases
    flag. Please note that this creates a temporary database, directly from IMGT, stored at /tempdbs/fasta/
    that will be deleted each time you run the analysis pipeline; if you would like to be able to re-use 
    this exact same database in the future, be sure to make a backup copy of the entire /tempdbs/fasta/
    folder. If you would like to curate your own database, or would like to continue using the most recent
    database generated by the --new_TR_databases flag, you can use the --use_tempdb flag in future analyses, 
    and the pipeline will default to whatever database is currently present in the /tempdbs/fasta/ folder.
    Currently only the /tempdbs/fasta/ folder is deleted, but other folders in the /tempdbs/ directory
    will remain.
...
if use_tempdb and new_TR_databases:
    print "Error: --use_tempdb and --new_TR_databases are exclusive of one another. Use --new_TR_databases to generate a new database, and then flag --use_tempdb in all future analyses if you want to continue using the most recent new_TR_databases. Note: if --use_tempdb is not flagged, the previous new_TR_databses will be deleted at the start of each analyses."
    sys.exit(1)
elif use_tempdb:
    if not os.path.isdir(path_to_scripts + "/tempdbs/fasta/"):
        print "Error: --use_tempdb can only be used if TCR databases exists in /tempdbs/fasta/ . Use --new_TR_databases to first generate these databases, and then flag --use_tempdb in future analyses to continue using the new databases. See the help notes for more information."
        sys.exit(1)
elif new_TR_databases:
    print "   ###############################################"
    print "   # Downloading and formatting new databases.   #"
    print "   # Please note that this database is temporary #"
    print "   # and will be deleted with next script run.   #"
    print "   ###############################################"
    cmd = "python {}/GetNewDBs.py".format(path_to_scripts)
    run( cmd )
else:
    if os.path.isdir(path_to_scripts + "/tempdbs/fasta/"):
        print "Removing old tempdbs/fasta/ directory."
        shutil.rmtree(path_to_scripts + "/tempdbs/fasta/", ignore_errors=False, onerror=None)
